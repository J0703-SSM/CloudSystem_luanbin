...
<html>
    ...
    <body>
        ...
        <div id="main">            
            ...
            <form action="/fee/updateCost" method="post" class="main_form">
                ...
                <div class="input_info fee_type">
                    <input type="radio" name="radFeeType"
                           <c:if test="${cost.unit_cost == null}"> checked </c:if> id="monthly" onclick="feeTypeChange(1);" />
                    <label for="monthly">包月</label>
                    <input type="radio" name="radFeeType"
                            <c:if test="${cost.base_duration != null and cost.base_cost != null and cost.unit_cost != null}"> checked</c:if> id="package" onclick="feeTypeChange(2);" />
                    <label for="package">套餐</label>
                    <input type="radio" name="radFeeType" <c:if test="${cost.base_cost == null}"> checked</c:if> id="timeBased" onclick="feeTypeChange(3);" />
                ...
                <div class="button_info clearfix">
                    ...
                    <input type="button" value="取消" class="btn_save" onclick="location.href='/fee/fee_list_findAll/1';"/>
                </div>
            </form>
        </div>
        <!--主要区域结束-->
    ...
    <script>
        window.onload = function(){
            if(${cost.unit_cost == null}){
                feeTypeChange(1);
            }
            if (${cost.base_duration != null and cost.base_cost != null and cost.unit_cost != null}){
                feeTypeChange(2);
            }

            if (${cost.base_cost == null}){
                feeTypeChange(3);
            }
        }
    </script>
