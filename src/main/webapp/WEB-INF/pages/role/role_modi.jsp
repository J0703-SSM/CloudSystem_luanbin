...
<div id="main">
    ...
    <form action="/role/updateRole" method="post" class="main_form">
        <div class="text_info clearfix"><span>角色名称：</span></div>
        <input type="hidden" name="role_id" value="${extRole.role_id}"/>
        <div class="input_info">
            <input type="text" class="width200" name="name" value="${extRole.name}"/>
            ...
            <div class="input_info_scroll">
                <ul>
                    <c:forEach items="${moduleInfos}" var="moduleInfo">
                        <li><input name="module_names" type="checkbox" value="${moduleInfo.name}"
                                <c:forEach items="${results}" var="result">
                                    <c:if test="${result !=null}">
                                        <c:if test="${result ==moduleInfo.name}">
                                            checked
                                        </c:if>
                                    </c:if>
                                </c:forEach>
